...
export default function RetailPOSPage() {
  ...
  return (
    // Changed: Used md:flex-row instead of xl:flex-row to trigger side-by-side on tablets. Adjusted gap sizes.
    <div className="h-full min-h-[calc(100vh-200px)] flex flex-col md:flex-row gap-4 lg:gap-6">
      ...
      <div className="flex-1 flex flex-col h-[600px] md:h-auto bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden print:hidden">
        ...
        <div className="p-4 lg:p-6 flex-1 overflow-y-auto bg-gray-50/30">
          {loading ? (
            ...
            // Changed: Reconfigured grid columns to 2 on md (since cart is next to it), 3 on lg+
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4 lg:gap-5">
              {currentProducts.map((product) => (
                <button
                  key={product.id}
                  onClick={() => addToCart(product)}
                  className="bg-white border border-gray-200 rounded-xl p-4 lg:p-5 text-left hover:border-indigo-500 hover:shadow-md transition-all active:scale-95 flex flex-col justify-between aspect-[4/3] group relative overflow-hidden"
                >
                  <div className="mb-2 z-10">
                    <p className="text-xs font-bold text-gray-400 mb-1">{product.sku}</p>
                    <h3 className="text-sm lg:text-base font-bold text-gray-900 group-hover:text-indigo-700 transition-colors line-clamp-2 leading-snug">{product.name}</h3>
                  </div>
                  <div className="z-10 mt-auto">
                    <span className="inline-block px-2 py-1 bg-indigo-50 text-indigo-700 text-[10px] lg:text-xs font-bold rounded mb-1 lg:mb-2">
                      Stock: {product.currentStock}
                    </span>
                    <p className="text-lg lg:text-xl font-black text-indigo-600">{formatCurrency(product.retailPrice)}</p>
                  </div>
                </button>
              ))}
            </div>
      ...
      {/* Cart Right Section */}
      {/* Changed: Added dynamic widths for md and lg, and fixed heights so it scrolls perfectly on tablets without pushing the whole page */}
      <div className="w-full md:w-[320px] lg:w-[360px] xl:w-[420px] flex flex-col bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden h-[500px] md:h-auto md:max-h-full print:hidden">
        <div className="p-4 lg:p-6 border-b border-gray-100 bg-indigo-600 text-white flex justify-between items-center shadow-inner">
          <h2 className="text-lg lg:text-xl font-bold">Current Bill</h2>
          <span className="bg-white/20 px-3 py-1 rounded-lg text-[10px] lg:text-xs font-bold text-white uppercase tracking-wider">Retail POS</span>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-3 lg:p-4 space-y-3">
          {cart.length === 0 ? (
            ...
            cart.map((item) => (
              <div key={item.product.id} className="flex items-center justify-between p-3 border border-indigo-100 bg-indigo-50/30 rounded-xl shadow-sm hover:shadow transition">
                <div className="flex-1 pr-2 lg:pr-3">
                  ...
                  <p className="text-xs lg:text-sm text-indigo-600 font-extrabold mt-1">{formatCurrency(item.product.retailPrice)}</p>
                </div>
                <div className="flex items-center gap-1">
                  ...
                  <input
                    ...
                    className="w-10 h-7 lg:w-14 lg:h-8 text-center font-bold text-gray-900 border border-gray-200 rounded-lg text-xs lg:text-sm bg-white outline-none focus:ring-2 focus:ring-indigo-400 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                  />
        ...
        <div className="p-4 lg:p-6 border-t border-gray-100 bg-gray-50 space-y-3 lg:space-y-4 shadow-inner">
          ...
          <button disabled={cart.length === 0 || isProcessing} onClick={handleConfirmOrder} className="w-full py-3 lg:py-4 bg-indigo-600 text-white font-bold text-lg lg:text-xl rounded-xl hover:bg-indigo-700 transition shadow-md hover:shadow-lg disabled:bg-gray-300 disabled:shadow-none disabled:cursor-not-allowed flex items-center justify-center gap-2 lg:gap-3">
